...
export class CRDTDocument {
  ...
  // Serialize the document for storage/network transmission
  serialize(): string {
    return JSON.stringify(this.inner)
  }

  // Deserialize and load a document
  static deserialize(agent: string, data: string): CRDTDocument {
    const doc = new CRDTDocument(agent)
    doc.inner = JSON.parse(data)
    return doc
  }

  // Create a copy of this document
  clone(): CRDTDocument {
    const copy = new CRDTDocument(this.agent)
    copy.inner = JSON.parse(JSON.stringify(this.inner))
    return copy
  }
}
